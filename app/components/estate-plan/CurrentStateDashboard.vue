<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      >
        ...
        <p class="text-lg font-semibold text-gray-900">
          {{ plan.grantor1.fullName }}
        </p>
        <p
          v-if="plan.grantor1.email"
          class="text-sm text-gray-500"
        >
          {{ plan.grantor1.email }}
        </p>
      ...
      >
        ...
        <p class="text-lg font-semibold text-gray-900">
          {{ plan.grantor2.fullName }}
        </p>
        <p
          v-if="plan.grantor2.email"
          class="text-sm text-gray-500"
        >
          {{ plan.grantor2.email }}
        </p>
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <p class="font-medium text-gray-900">
            {{ formatTrustType(plan.trust.trustType) }}
          </p>
        </div>
        <div>
          <p class="text-sm text-gray-500">
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="border-l-4 border-blue-400 pl-4">
          ...
          <div class="space-y-2 text-sm">
            ...
            >
              ...
              <UiBadge
                v-if="getPrimaryWill.executionDate"
                size="sm"
                variant="success"
              >
                Executed
            ...
            <div
              v-for="doc in getPrimaryAncillaryDocs"
              :key="doc.id"
              class="flex items-center gap-2"
            >
              <FileText class="w-4 h-4 text-gray-400" />
        ...
        <div class="border-l-4 border-purple-400 pl-4">
          ...
          <div class="space-y-2 text-sm">
            <div
              v-if="getSecondaryWill"
              class="flex items-center gap-2"
            >
              <FileText class="w-4 h-4 text-gray-400" />
              <span>{{ formatWillType(getSecondaryWill.willType) }}</span>
              <UiBadge
                v-if="getSecondaryWill.executionDate"
                size="sm"
                variant="success"
              >
                Executed
            ...
            <div
              v-for="doc in getSecondaryAncillaryDocs"
              :key="doc.id"
              class="flex items-center gap-2"
            >
              <FileText class="w-4 h-4 text-gray-400" />
    ...
    <div
      v-else-if="plan.wills.length > 0 && plan.wills[0]"
      class="bg-white border border-gray-200 rounded-lg p-4"
    >
      <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">
        Will Information
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          ...
          <p class="font-medium text-gray-900">
            {{ formatWillType(plan.wills[0]?.willType ?? null) }}
          </p>
        </div>
        <div>
          <p class="text-sm text-gray-500">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div v-if="successorTrustees.length > 0">
          ...
          <div
            v-for="role in successorTrustees"
            :key="role.id"
            class="text-sm"
          >
            <span class="font-medium text-gray-900">{{ role.person?.fullName || 'Unknown' }}</span>
        ...
        <div v-if="primaryBeneficiaries.length > 0">
          ...
          <div
            v-for="role in primaryBeneficiaries"
            :key="role.id"
            class="text-sm"
          >
            <span class="font-medium text-gray-900">{{ role.person?.fullName || 'Unknown' }}</span>
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="border-l-4 border-blue-400 pl-4">
          <h4 class="font-medium text-gray-900 mb-3">
            {{ plan.grantor1?.fullName }}'s Agents
          </h4>
          <div class="space-y-2 text-sm">
        ...
        <div class="border-l-4 border-purple-400 pl-4">
          <h4 class="font-medium text-gray-900 mb-3">
            {{ plan.grantor2?.fullName }}'s Agents
          </h4>
          <div class="space-y-2 text-sm">
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div v-if="executor">
          ...
          <p class="font-medium text-gray-900">
            {{ executor.person?.fullName || 'Unknown' }}
          </p>
        </div>
        <div v-if="financialAgent">
          ...
          <p class="font-medium text-gray-900">
            {{ financialAgent.person?.fullName || 'Unknown' }}
          </p>
        </div>
        <div v-if="healthcareAgent">
          ...
          <p class="font-medium text-gray-900">
            {{ healthcareAgent.person?.fullName || 'Unknown' }}
          </p>
        </div>
      </div>
    </div>
