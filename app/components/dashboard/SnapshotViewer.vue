  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
        ...
        <div class="flex items-center space-x-3">
          <IconCheckCircle
            :class="snapshot.approved_by_council ? 'text-green-500' : 'text-gray-300'"
            class="w-6 h-6"
          />
          <div>
            <div class="text-sm font-medium text-gray-900">Council Approval</div>
            <div class="text-xs text-gray-600">
              {{ snapshot.approved_by_council ? 'Approved' : 'Pending' }}
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Feedback/Notes -->
    <div v-if="snapshot.client_feedback || snapshot.council_notes" class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
      ...
      <div v-if="snapshot.council_notes" class="mt-3 pt-3 border-t border-yellow-300">
        <div class="flex items-start">
          <IconMessageSquare class="w-5 h-5 text-yellow-700 mr-2 mt-0.5" />
          <div>
            <div class="text-sm font-medium text-yellow-900">Council Notes</div>
            <div class="text-sm text-yellow-800 mt-1">{{ snapshot.council_notes }}</div>
          </div>
        </div>
      </div>
    </div>
...
const isApproved = computed(() => {
  ...
  return props.snapshot.approved_by_council
})
