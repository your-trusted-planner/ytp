...
export default defineEventHandler(async (event) => {
  // Only admins and lawyers can approve time entries
  ...
  if (existing.status !== 'SUBMITTED') {
    throw createError({
      statusCode: 400,
      message: `Cannot approve a time entry with status '${existing.status}'. Only SUBMITTED entries can be approved.`
    })
  }

  // Cannot approve your own entries (unless super admin)
  if (existing.userId === user.id && user.adminLevel < 3) {
    throw createError({
      statusCode: 403,
  ...
  // Fetch updated entry with user info
  ...
  return {
    timeEntry: {
      ...
      approverName: [user.firstName, user.lastName].filter(Boolean).join(' ') || 'Unknown',
