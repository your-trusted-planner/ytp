...
export async function seedDatabase(db: DrizzleD1Database<typeof schema>) {
  ...
  // Create sample service catalog items (products/services)
  ...
  console.log('✅ Created sample service catalog items')

  // Create a sample matter (client case) for the test client
  const matter1Id = generateId()
  await db.insert(schema.matters).values({
    id: matter1Id,
    clientId,
    title: 'Smith Family Trust Formation',
    description: 'Wyoming Asset Protection Trust for the Smith family',
    status: 'OPEN',
    createdAt: new Date(),
    updatedAt: new Date()
  })

  console.log('✅ Created sample client matter')
